...
class PixelAnimationOrchestratorTest {
    ...
    @Test
    fun `orchestrator resyncs the active variant when controller clip drifts`() {
        val controller = PixelAnimationController()
        val happyVariant = loopVariant(id = "happy_loop")
        val neutralVariant = loopVariant(id = "neutral_loop")
        val orchestrator = PixelAnimationOrchestrator(
            stateMapper = DefaultPixelPetStateMapper(),
            animationSetRegistry = registry(
                neutralVariants = listOf(neutralVariant),
                happyVariants = listOf(happyVariant)
            ),
            variantSelector = PixelAnimationVariantSelector(
                strategy = PixelVariantSelectionStrategy.SimpleRoundRobin,
                random = Random(3)
        ...
        orchestrator.synchronize(PixelPetBridgeState(PixelPetAvatarIntent.ENGAGED))
        controller.setClip(neutralVariant.clip)

        val resyncedVariant = orchestrator.synchronize(PixelPetBridgeState(PixelPetAvatarIntent.ENGAGED))
        val diagnostics = orchestrator.getDiagnostics()

        assertSame(happyVariant, resyncedVariant)
        assertEquals("happy_loop_clip", controller.getPlaybackState()?.clip?.id)
        assertEquals(1, diagnostics.clipResyncCount)
        assertEquals(0, diagnostics.stateTransitionCount)
    }
